		...
		<div class="modal-box">
			...
			<div class="col-sm-6">
				<div class="modal_content" style="margin: 10%">
					<div>
						<p>
							현재 유저의 권한은 [ <span style="font-style: italic; font-weight: bold;"> ${reportType.user.role} </span> ] 입니다.
						</p>
						<p>계정을 제재하시겠습니까?</p>
					</div>
				</div>
									...
									<tbody>
										...
										<c:choose>
											<c:when test="${report.reportType eq 'board'}">
												<tr>
													<th>제목</th>
													<td>${reportType.title}<a href="/board/detail/${reportType.id}">[해당 게시글로 이동하기]</a></td>
												</tr>
												<tr>
													<th>내용</th>
												...
												<tr>
													<th>사진</th>
													<td><img style="width: 50px; height: 50px;" src="/upload/${reportType.image1}" alt=" No Image" />
														</li> <c:if test="${!empty reportType.image2}">
															<li><img style="width: 50px; height: 50px;" src="/upload/${reportType.image2}" alt=" No Image" /></li>
														</c:if> <c:if test="${!empty reportType.image3}">
															<li><img style="width: 50px; height: 50px;" src="/upload/${reportType.image3}" alt=" No Image" /></li>
														</c:if> <c:if test="${!empty reportType.image4}">
															<li><img style="width: 50px; height: 50px;" src="/upload/${reportType.image4}" alt=" No Image" /></li>
														</c:if> <c:if test="${!empty reportType.image5}">
															<li><img style="width: 50px; height: 50px;" src="/upload/${reportType.image5}" alt=" No Image" /></li>
														</c:if></td>
												</tr>
											</c:when>
											<c:when test="${report.reportType eq 'comment'}">
												<tr>
													<th>내용</th>
													<td>${reportType.content}<a href="/board/detail/${reportType.board.id}">[해당 게시글로 이동하기]</a></td>
												</tr>
											</c:when>
